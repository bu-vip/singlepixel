...
class TCS34725:
    ...
    def __readList(self, reg, length):
        "Read a list of bytes from the I2C device"
        return self.i2c.readList(self.__TCS34725_COMMAND_BIT | reg, length)
...
doing anything else)"
        ...
        if (result != 0x44):
            #print("The Curse of the Undying Valid Bit")
            return -1
        else:
            self.valid = True
    ...
    def enable(self):
	#Lawrence: Modified to not flash LED
        self.__write8(self.__TCS34725_ENABLE, (self.__TCS34725_ENABLE_PON | self.__TCS34725_ENABLE_AIEN))
        time.sleep(0.01)
        self.__write8(self.__TCS34725_ENABLE, (self.__TCS34725_ENABLE_PON | self.__TCS34725_ENABLE_AEN | self.__TCS34725_ENABLE_AIEN))
    ...
    def getID(self):
        return self.__readU8(self.__TCS34725_ID)
    ...
    def readList2(self, reg, length):
        ...
        try:
          ...
          return self.errMsg()
    '''
    def getRawDataList(self): #Added 5/18 by Hayleigh, reads single data stream for less overhead
        "Reads the raw red, green, blue and clear channel values"
        color = {}
        colors = self.__readList(self.__TCS34725_CDATAL, 8)
        color["c"] = colors[1]*256 + colors[0];
        color["r"] = colors[3]*256 + colors[2];
        color["g"] = colors[5]*256 + colors[4];
        color["b"] = colors[7]*256 + colors[6];
        # Set a delay for the integration time
        #delay = self.__integrationTimeDelay.get(self.integrationTime)
        #time.sleep(delay)
        #print colors
