...
import time
from Adafruit_TCS34725_BaldEagles3 import TCS34725
import Adafruit_I2C
import numpy
...
def ResetChannels():
        ...
        for u in range(0, 8):
                ...
                if mux != '--':
                    mux = '0x' + mux
                    mux = int(mux, 16)
                    bus.write_byte_data(mux, DEVICE_REG_MODE1, 0x00) #Choose no channels
                    print "mux ", mux, " is clear"
                            
                
#ColorSensor_robdef="""
ColorSensor_interface="""
service ColorSensor_interface

option version 0.4

object ColorSensor
        function double[4] ReadSensor()
        function int32[*] ReadSensors()
        
end object
"""

class ColorSensor_impl(object):
        def start(self):
                pass
...
def readList2(self, reg, length):
        ...
        try:
          results = bus.read_i2c_block_data(self.address, reg, length)
          if self.debug:
...
def ReadSensors():
        ...
        for u in range(0, 8):
                mux = muxes[u]
                if mux != '--' and mux_present[u]:
                        mux = '0x' + mux
                        mux = int(mux, 16)
                        x2 = 1
                        print sensor_present
                        print " "
                        for s2 in range(0, 8):
                                bus.write_byte_data(mux, DEVICE_REG_MODE1, x2) #Choose sensor x on mux m
                                #tcs = TCS34725(integrationTime=0xD5, gain=0x03) # 2.4 ms integration time, 60X amp
                                #sleep(.1)
                                if (sensor_present[u][s2]):
                                        #tcs.enable()
                                        #sleep(.1)
                                        ...
                                        x2 = x2*2
                        bus.write_byte_data(mux, DEVICE_REG_MODE1, 0x00) #Choose sensor x on mux m
        print data
...
def main():
        ...
        RR.RobotRaconteurNode.s.RegisterServiceType(ColorSensor_interface)
        ...
        RR.RobotRaconteurNode.s.RegisterService("ColorSensor","ColorSensor_interface.ColorSensor",o)
        t.StartServer(2335)
...
ResetChannels()
muxes = i2cdetect_mux()
sensor_present = [[0 for q in range(8)] for q in range (8)] # initialize 8x8 data matrix
x = 1
mux_present = [0 for q in range(8)]
counter = 0
for m in range(FIRST_MUX_ADDRESS, LAST_MUX_ADDRESS + 1):
        x = 1
        for s in range(0, 8):
                try:
                        bus.write_byte_data(m, DEVICE_REG_MODE1, x)
                        mux_present[m - FIRST_MUX_ADDRESS] = True
                        
                        print "Mux present at: " + hex(m)
                except IOError, err:
                        print  "There's no mux at this address: " + hex(m)
                        mux_present[m - FIRST_MUX_ADDRESS] = False
                ...
                print id
                if (id == 0x44):
                        sensor_present[m - FIRST_MUX_ADDRESS][s] = 1
                else:
                        sensor_present[m - FIRST_MUX_ADDRESS][s] = 0
                sleep(.03)
                counter += 1
        if(mux_present[m - FIRST_MUX_ADDRESS]):
                bus.write_byte_data(m, DEVICE_REG_MODE1, 0x00) #Choose sensor x on mux m
        sleep(.1)
print "Init complete"
print sensor_present
